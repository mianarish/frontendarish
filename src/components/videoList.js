...
const FeedPage = () => {
  ...
  const hasAudio = async (videoUrl) => {
    try {
      ...
      return contentType.includes('audio/') || contentType.includes('video/'); // Check for video files with audio
    } catch (error) {
  ...
  const handleAudioDownload = async () => {
    ...
    if (videoUrl) {
      try {
        console.log("Attempting to download audio from:", videoUrl);
        const audioExists = await hasAudio(videoUrl);

        if (audioExists) {
          console.log("Audio detected. Downloading MP3...");
          const response = await fetch(videoUrl);
          if (!response.ok) {
            console.error(`Failed to fetch video. Status: ${response.status}`);
            throw new Error(`HTTP error! status: ${response.status}`);
          ...
          const link = document.createElement("a");
          link.href = blobUrl;
          link.download = `audio-${currentIndex + 1}.mp3`; // Download as MP3
          link.click();
  ...
  return (
    ...
    >
      ...
      >
        ...
        <IconButton sx={{ color: "#64b5f6" }} onClick={handleAudioDownload}>
          <MusicNoteIcon />
        </IconButton>
